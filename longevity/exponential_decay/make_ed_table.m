...
addParameter(parseobj, 'normalize_initial_value', true, @(x) isscalar(x)&&(x==0||x==1))
...
for i = 1:max(T.condition)
    ...
    [p_hat, p_CI, p_boot] = fit_sum_2_exp_decay(x,y, ...
                        'n_boot', P_in.n_boot, ...
                        'normalize_initial_value', P_in.normalize_initial_value, ...
                        'x0', P_in.x0);
    t_half_boot = compute_model_half_life(p_boot);
    t_half_boot = double(t_half_boot);
    T_ed.t_half(i,1) = median(t_half_boot);
