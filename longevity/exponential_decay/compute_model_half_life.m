% COMPUTE_MODEL_HALF_LIFE compute the half-life of the a model of the sum
...
function t_half = compute_model_half_life(p, varargin)
P=get_parameters;
...
for i = 1:size(p,1)
    ...
    tau_f = p(i,3);
    tau_s = p(i,4);
    eqn = 0.5 == alpha*(exp(-1/tau_f))^x + (1-alpha)*(exp(-1/tau_s))^x;
