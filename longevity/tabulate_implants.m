...
% CSV file
    ...
    for i = 1:numel(penetrations)
        if ~ismember(penetrations(i).rat(i), P.rats)
            continue
        end
        k = k + 1;
        ...
        if penetrations(i).probe_orientation == 0
            T.shank_plane(k,1) = "sagittal";
        elseif  penetrations(i).probe_orientation == 90
            T.shank_plane(k,1) = "coronal";
        else
            error('unknown shank orientation')
        end
...
function age = get_age_at_time_of_implant(rat_name, date_implanted)
    ...
    age = days(delivery_date - date_implanted);
end
