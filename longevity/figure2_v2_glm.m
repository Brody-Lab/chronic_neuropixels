%figure2_v2_glm
tic
S = fit_exp_to_electrodes(Cells, 'hierarch', false);
toc
b =[S.b];
%%
...
beta_text = cellfun(@(x) ['e^{\beta' num2str(x) '}'], num2cell(1:5), 'uni', 0);
...
xticklabel = cellfun(@(x) P.text.(x), P.exp_decay_regressors, 'uni', 0);
beta_text = cellfun(@(x) ['\beta_{' num2str(x) '}'], num2cell(7:11), 'uni', 0);
xticklabel = cellfun(@(x,y) [x,' \newline(' y ')'], xticklabel, beta_text, 'uni', 0);
