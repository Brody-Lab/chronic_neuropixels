...
%       The alpha for elastic net regularization, between 0 and 1. A value
%       of 0 is ridge regularization, and 1 is lasso regularization. If
%       it's empty, not regularization will be performed.
%
%   regressors
...
P = get_parameters;
addParameter(parseobj, 'hierarchical_bootstrap',true, @(x) isscalar(x) && (x==0 ||x==1))
...
if P_in.hierarchical_bootstrap
    ...
    while i<P_in.n_boot
        ...
        if isempty(P_in.regularization_alpha)
            [b, dev, stats] = glmfit(X(boot_idx,:), y(boot_idx), 'poisson', 'link', 'log');
        else
            [b, dev, stats] = glmfit(X(boot_idx,:), y(boot_idx), 'poisson', 'link', 'log');
        end
        if isempty(lastwarn)
    ...
    for i = 1:max(P_in.n_boot,1)
        idx = boot_idx(i,:)';
        if P_in.return_stats
            [b, dev, stats] = glmfit(X(idx,:), y(idx), 'poisson', 'link', 'log');
            S(i).b = b;
            S(i).dev = dev;
            S(i).stats = stats;
            S(i).boot_idx = idx;
        else
            S(i).b = glmfit(X(idx,:), y(idx), 'poisson', 'link', 'log');
        end
    end
end