function []=plot_t_half(T_ed, varargin)
    P=get_parameters;
    ...
    medians = T_ed.t_half(idx);
    upper_err = T_ed.t_half_CI(idx,2) - medians;
    lower_err = medians-T_ed.t_half_CI(idx,1);
    ...
    title(P.text.(P_in.metric), 'fontweight', 'normal')
    if P_in. ylabel_on
        ylabel('Model half-life (days)')
    end